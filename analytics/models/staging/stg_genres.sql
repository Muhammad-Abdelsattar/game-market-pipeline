with source as ( 
    select 
        * 
        ...
        {% endif %}
    from {{ source('rawg_lake', 'raw_genres') }} 
),
flattened as (
    select
...
select
    {{ dbt_utils.generate_surrogate_key(["'RAWG'", json_select('item', 'id')]) }} as genre_key,
    'RAWG' as source_system,
    {{ json_select('item', 'id') }}::int as genre_id,
    
    {{ json_select('item', 'name') }}::text as genre_name,
    {{ json_select('item', 'slug') }}::text as genre_slug,

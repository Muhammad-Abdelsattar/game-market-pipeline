...
select
    {{ dbt_utils.generate_surrogate_key(["'RAWG'", json_select('item', 'id')]) }} as developer_key,
    'RAWG' as source_system,
    {{ json_select('item', 'id') }}::int as developer_id,
    
    {{ json_select('item', 'name') }}::text as developer_name,
    {{ json_select('item', 'slug') }}::text as developer_slug,
